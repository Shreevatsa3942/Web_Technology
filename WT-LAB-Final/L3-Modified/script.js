...
var ugCGPA=document.getElementById('ug');
var pgCGPA=document.getElementById('pg')
...
function validation(){
    ...
    var ugCGPAValue=ugCGPA.value.trim();
    var pgValue=pgtitle.value.trim();
    var pgCGPAValue=pgCGPA.value.trim();
    ...
    if(currentTab === 0 ){
        ...
        if(currentTab === 4){
            if((ugValue !== '' && pgValue !== '' && prj1NameValue !== '' && prj1DescValue !== '' && ugCGPAValue !== '' && pgCGPAValue !== '')&& validateUGtitle() && validatePGtitle() && validateCGPA(ugCGPA,8) && validateCGPA(pgCGPA,9)){
                nextTab();
                displayAll();
            }
            else{
...
function nextTab(){
    ...
    if((currentTab > 0) && (currentTab < 7)){
        ...
        if(currentTab == 6){
            ...
            button[1].style.display="none";
            progress[currentTab].style.backgroundColor="green"
            div[currentTab].innerHTML="<h3 align='center'>Registration Successful</h3>"
        }
    }
}
function prevTab(){
...
function displayAll(){
    var Finalfullname=document.getElementById('Finalfullname');
    var Finalusername=document.getElementById('Finalusername');
    var Finalgender=document.getElementById('Finalgender');
    var Finalnationality=document.getElementById('Finalnationality');
    var Finaldob=document.getElementById('Finaldob');
    var Finalemail=document.getElementById('Finalemail');
    var Finalphno=document.getElementById('Finalphno');
    var Finalugtitle=document.getElementById('Finalugtitle');
    var Finalugscore=document.getElementById('Finalugscore');
    var Finalpgtitle=document.getElementById('Finalpgtitle');
    var Finalpgscore=document.getElementById('Finalpgscore');

    Finalfullname.value=firstname.value.trim()+" "+lastname.value.trim();
    Finalusername.value=username.value.trim();
    Finalgender.value=gender.value.trim();
    Finalnationality.value=nationality.value.trim();
    Finaldob.value=dob.value.trim();
    Finalemail.value=email.value.trim();
    Finalphno.value=phno.value.trim();
    Finalugtitle.value=ugtitle.value.trim();
    Finalugscore.value=ugCGPA.value.trim();

    console.log(pgtitle.value);
    Finalpgtitle.value=pgtitle.value.trim();
    Finalpgscore.value=pgCGPA.value.trim();
    //console.log(Finalfullname.value);
}